...
| ⚖️ **法律** | 🏥 **医疗** | 💼 **金融** |
|--------------|-------------------|----------------|
| - 案件结果预测<br>- 合同条款分类<br>- 法律推理 / 多跳问答 | - 放射报告异常分类<br>- 临床记录编码<br>- 医学命名实体识别 / 药物相互作用 | - 情感分析<br>- 风险评估<br>- 欺诈检测 / 金融问答 |
...
|---------------------------|---------------------------|----------------------|-------------------|
| 在将 LLM 调用降至接近零的同时保持接近教师模型的质量 | 学生模型处理大部分查询；仅在不确定时调用教师模型 — 推理费用大幅下降 | 本地学生模型推理 vs LLM（1T 参数）API 调用 — 延迟上数量级加速 | 最少代码改动 — 几行代码即可运行 |

**学生模型说明：** 学生流水线 = **冻结编码器**（默认 DistilBERT ~6600 万参数）+ **可训练分类头**（DeepMLP 或 GCP，约 10 万–200 万参数，视配置而定）。"1000× 加速"指**延迟**（本地前向传播 vs LLM API 往返）；成本削减见 [成本削减](#-成本削减)。
